...
import com.example.capstone_jpa.Service.MerchantService;
...
public class MerchantController {
    ...
    public ResponseEntity<?> setDiscount(@RequestParam Integer productId,
                                         @RequestParam double discount) {
        boolean ok = productService.setProductDiscount(productId, discount);
        if (ok) {
            return ResponseEntity.status(200).body(new ApiResponse("Discount set successfully"));
        }

...
public class MerchantStockController {
    ...
    public ResponseEntity<?> updateStock(@PathVariable String id, @Valid @RequestBody MerchantStock merchantStock, Errors errors) {
        if (errors.hasErrors()) {
            return ResponseEntity.status(400).body(errors.getFieldError().getDefaultMessage());
        }
    ...
    @PutMapping("/addStock")
    public ResponseEntity<?> addStock( String merchantId,  String productId,  int stock) {
